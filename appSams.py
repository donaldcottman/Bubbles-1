...
def buyStock(stock, shareQuantity, current_price, deltaP):
    ...
    stockDeltaP = deltaP
    print(f"current_price FROM BUY DEF {stock} {current_price}")
    ...
    if stockDeltaP <= 6:
        ...
        rh.orders.order_sell_trailing_stop(stockName, shareQuantity, round(stockDeltaP*0.1*0.33, 2), trailType='percentage', timeInForce='gfd', extendedHours=True, jsonify=True)
    else:
        rh.orders.order_sell_trailing_stop(stockName, shareQuantity, round(stockDeltaP*0.1*2, 2), trailType='percentage', timeInForce='gfd', extendedHours=True, jsonify=True)
...
def run_script():
    ...
    def run_bubbles(result_list, currentCPUProcess, maxCPUProcesses):
        ...
        def bubblesUpdate(stock, stockVolume):
            global topStockData
            # Stock symbol
            symbol = stock
            ...
            if (stockVolume == 0):
                current_volume = hist_data_totalVolume["results"][0]["v"] #hist_data['v'][-1] #50-random_number
            else:
        ...
        def JSONToBeSentToApp(stocksSorted):
            ...
            for i in range(len(stocksSorted)):
                ...
                bubblesUpdate_result = bubblesUpdate(stocksSorted[i]['stock'], stocksSorted[i]['volume'])
    ...
    responseCompiled = {str(int(time.time())): response_Sorted}
